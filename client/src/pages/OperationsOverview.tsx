...
import { Activity, AlertTriangle, ArrowUpRight, Building2, CheckCircle2, ClipboardCheck, Database, FileUp, Gauge, Radio, Settings2 } from "lucide-react";
...
export default function OperationsOverview() {
  ...
  return <div className="ecosystem-page">
    ...
    <section className="overview-metrics" aria-label="Operational tenant metrics">
      ...
      <article><Gauge size={20}/><span>ECOSCORE</span><strong>{monitoring?.latestScore?.score ?? "—"}</strong><small>{monitoring?.latestRun ? `${monitoring.latestRun.status} · ${monitoring.openAlertCount} open alert${monitoring.openAlertCount === 1 ? "" : "s"}` : "run monitoring to create a score"}</small></article>
    </section>
