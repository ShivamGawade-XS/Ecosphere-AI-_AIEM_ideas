...
export default function IngestionWorkbench() {
  ...
  const ingestReading = trpc.readings.ingest.useMutation({ onSuccess: async (result) => { if (organizationId) await utils.ingestion.recent.invalidate({ organizationId }); setNotice(result.idempotent ? "This reading was already accepted; duplicate data was not created." : "Reading accepted and recorded with provenance."); } });
  ...
  return <div className="workbench-canvas">
    ...
    <section className="workbench-flow">
      ...
      <form onSubmit={submitReading} className="workbench-step workbench-step--accent"><span>04 · INGEST</span><h2>Reading</h2><p>Submit one validated, idempotent measurement to the selected meter.</p><select value={meterId ?? ""} onChange={(event) => setMeterId(Number(event.target.value))} aria-label="Target meter"><option value="" disabled>Select a meter</option>{metersQuery.data?.map((meter) => <option value={meter.id} key={meter.id}>{meter.displayName} · {meter.canonicalUnit}</option>)}</select><input value={readingValue} onChange={(event) => setReadingValue(event.target.value)} aria-label="Reading value" type="number" min="0" step="0.01" required /><button disabled={!meterId || isWorking}>{ingestReading.isPending ? <Loader2 className="animate-spin" /> : <Send />} Ingest reading</button></form>
    </section>
