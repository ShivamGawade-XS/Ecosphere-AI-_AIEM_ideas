...
export default function RegistryWorkspace() {
  ...
  const busy = createOrganization.isPending || createSite.isPending || createMeter.isPending || updateMemberRole.isPending;
  ...
  return <div className="ecosystem-page"><header className="workspace-header"><div><span className="ops-eyebrow"><span /> REGISTRY</span><h1>Define what can be measured.</h1><p>Every live reading is scoped to an organization, a site, a meter, a resource type, and a canonical unit.</p></div><div className="tenant-badge"><Database size={18}/><span>{tenant.activeOrganization?.organization.name ?? "No tenant"}</span><b>Foundation</b></div></header>{notice && <div className="workbench-notice" role="status">{notice}</div>}
    ...
    <section className="evidence-board" aria-label="Tenant access administration"><header><div><span className="ops-eyebrow">ACCESS GOVERNANCE</span><h2>Tenant members and roles</h2></div><Building2 size={23}/></header><p className="muted">Managers and owners can review the existing identity-provider members. Only an owner can change roles, and the final owner cannot be demoted. Invitation delivery is not configured in this deployment.</p>{roleChangeError && <div className="workbench-notice" role="alert">{roleChangeError}</div>}{!organizationId ? <p className="muted">Create an organization before access assignments can be reviewed.</p> : membersQuery.isLoading ? <p className="muted" role="status">Loading tenant members…</p> : membersQuery.error ? <p className="muted">Member visibility requires a manager or owner role. No member records are inferred when authorization is denied.</p> : membersQuery.data?.length ? <div className="evidence-columns">{membersQuery.data.map(({ membership, user }) => <div key={membership.id}><h3>{user.name ?? user.email ?? `User ${user.id}`}</h3><p><span>{user.email ?? "Identity-provider user"} · joined {new Date(membership.createdAt).toLocaleDateString()}</span>{canManageRoles ? <select aria-label={`Role for ${user.name ?? user.email ?? `user ${user.id}`}`} value={membership.role} disabled={busy} onChange={(event) => updateMemberRole.mutate({ organizationId, memberUserId: user.id, role: event.target.value as "owner" | "manager" | "operator" | "viewer" })}><option value="owner">Owner</option><option value="manager">Manager</option><option value="operator">Operator</option><option value="viewer">Viewer</option></select> : <b>{membership.role}</b>}</p></div>)}</div> : <p className="muted">No tenant members are recorded yet.</p>}</section>
  </div>;
}
