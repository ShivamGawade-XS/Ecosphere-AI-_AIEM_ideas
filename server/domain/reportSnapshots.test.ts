...
describe("materializeReportSnapshot", () => {
  it("pins tenant scope, bounds evidence collections, and discloses governed factors", () => {
    const result = materializeReportSnapshot({ organizationId: 8, generatedAt: new Date("2026-08-22T00:00:00.000Z"), overview: { siteCount: 1 }, monitoring: { openAlertCount: 0 }, forecasts: Array.from({ length: 11 }, (_, id) => ({ id })), recommendations: Array.from({ length: 21 }, (_, id) => ({ id })), comparisons: Array.from({ length: 11 }, (_, id) => ({ id })), targetAssessments: Array.from({ length: 31 }, (_, id) => ({ id })), scenarios: Array.from({ length: 11 }, (_, id) => ({ id })), latestDemoSimulation: { status: "spike_injected", simulationVersion: "aiem-campus-demo-v1", anchorObservedAt: new Date("2026-08-21T08:00:00.000Z") }, approvedFactors: [{ factorVersion: "goa-grid-v1", sourceName: "Campus factor library" }] });
    ...
    expect(result.evidence.targetAssessments).toHaveLength(30);
    expect(result.evidence.scenarios).toHaveLength(10);
    expect(result.evidence.demoSimulation).toMatchObject({ explicitlySimulated: true, status: "spike_injected" });
