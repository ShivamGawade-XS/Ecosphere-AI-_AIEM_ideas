...
describe("buildMovingAverageForecast", () => {
  ...
  it("uses a deterministic rolling average and records holdout error evidence", () => {
    const forecast = buildMovingAverageForecast({
    ...
    expect(forecast.points).toEqual([
      { step: 1, predictedValue: 45 },
      { step: 2, predictedValue: 48.75 },
    ]);
    expect(forecast.backtest).toEqual({ evaluatedPointCount: 2, meanAbsoluteError: 25, meanAbsolutePercentageError: 45.8333 });
  });
