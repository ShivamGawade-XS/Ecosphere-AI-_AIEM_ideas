export function materializeReportSnapshot(input: {
  ...
  return {
    criteria: { organizationId: input.organizationId, scope: "current tenant-bound persisted records", generatedAt: input.generatedAt.toISOString(), version: "evidence-snapshot-v1" },
    evidence: { overview: input.overview, monitoring: input.monitoring, forecasts: input.forecasts.slice(0, 10), recommendations: input.recommendations.slice(0, 20), comparisons: input.comparisons.slice(0, 10) },
    factorDisclosure,
  };
}
