...
import { activateScheduledMonitoring, createSimulation, getCampusDashboard, importApprovedCsvTelemetry, injectEnergySpike, runScheduledMonitoring, setAlertStatus, updateMonitoringPreferences } from "../sustainability";
...
export const sustainabilityRouter = router({
  ...
  advisor: publicProcedure.input(advisorSchema).mutation(async ({ input }) => {
    ...
    return { content: fallback, source: "deterministic-fallback" as const };
  }),
});
