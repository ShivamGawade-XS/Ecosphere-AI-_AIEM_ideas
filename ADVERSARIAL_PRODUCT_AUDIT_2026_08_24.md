...
| P-08 | Resolved locally | Reports now offers a tenant-scoped Executive Evidence View generated solely from a stored snapshot, with target attainment, scenario/SDG contribution, comparison/recommendation counts, factor disclosure, explicit demo provenance, snapshot-bound CSV/JSON export, and print control. Print output includes a persistent tenant/snapshot/modeling/simulation disclosure header. | A judge or stakeholder can inspect a concise evidence presentation without a client-side recalculation or client-clock export timestamp. | Browser print is not a certified report, and stored modeled/simulated values must remain clearly labeled in any downstream document. |
