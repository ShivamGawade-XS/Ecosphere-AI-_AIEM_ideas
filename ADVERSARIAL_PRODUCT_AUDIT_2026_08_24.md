...
| P-06 | Medium | Recommendation and comparison algorithms are intentionally deterministic but narrow: fixed ranking weights, short moving average, and shallow action materialization. | The product can look more intelligent than its current decisions justify if claims are not tightly scoped. | Make formula versions, confidence/insufficiency, ranking weight explanations, and evidence constraints first-class in the UI; add optional manual weighting only if traceability remains intact. |
