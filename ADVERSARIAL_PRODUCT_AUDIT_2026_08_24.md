...
> **Historical baseline: 70/100. Current locally verified repository assessment: 78/100.** The codebase now has a server-owned, clearly simulated guided campus demo; deterministic SDG and target evidence; structured auth redirects; bounded worker history and database queueing; generic-ingestion isolation from simulated fixtures; snapshot-bound export/print evidence; a protected persisted-event traceability timeline; and deterministic forecast candidate selection with holdout evidence. The local full quality gate passed with **44 test files / 178 tests**, TypeScript, production build, production-dependency audit, and whitespace check. It is **not** a 100/100 production platform or enterprise-certified system: production database/OAuth setup, externally validated tenant flows, activated scheduling, restore rehearsal, distributed rate limiting, independent security testing, and a live IoT pilot remain deliberately unverified external gates.
...
|---|---|---|---|---|
| P-01 | Resolved locally | [server/demo/simulation.ts](server/demo/simulation.ts) and [server/domain/demoSimulation.ts](server/domain/demoSimulation.ts) implement tenant-scoped baseline setup, bounded cycles, a fixed 260 kWh HVAC spike, pipeline refresh, and reset of demo-owned evidence only. Generic protected ingestion now rejects `source: simulated`, so resettable fixtures can originate only from the guided-demo service. | The core presenter loop is now real server-owned functionality, not a browser timer or casually submitted simulated record. | Obtain authenticated browser and production-operator evidence before claiming deployed demo operation. |
...
| P-03 | Resolved locally | The signed-out bridge explains the protected workspace, provides sign-in and public-narrative escape routes, and preserves a branded fallback. Optional redirect paths are now restricted to same-origin public paths, preventing protected-route loop targets. | A judge is no longer presented with an unexplained protected-area failure or a local redirect trap. | Complete real OAuth browser-flow verification after operator configuration. |
...
| P-06 | Resolved locally | Comparison v2 persists each normalized weighted component and its point contribution; the Scenario UI surfaces these components. Recommendation evidence records a severity-tier method, attached evidence, and limitations; reusable pilot templates preserve user baselines and disclose that they are not procurement estimates. Forecasting now compares bounded moving-average and last-value candidates under the same rolling holdout MAE and persists the selected method plus selection rationale. | A reviewer can inspect why a rank, confidence label, or forecast candidate exists instead of treating it as opaque AI. | These remain simple pilot methods, not calibrated forecasts, vendor pricing, or autonomous intervention selection; do not imply predictive accuracy. |
...
| P-08 | Resolved locally | Reports now offers a tenant-scoped Executive Evidence View generated solely from a stored snapshot, with target attainment, scenario/SDG contribution, comparison/recommendation counts, factor disclosure, explicit demo provenance, snapshot-bound CSV/JSON export, and print control. The Intelligence workspace additionally presents a bounded protected timeline of stored quality, anomaly, alert, recommendation, and action events; it labels missing links rather than inventing lineage. | A judge or stakeholder can inspect a concise evidence presentation and follow persisted operational events without a client-side recalculation or client-clock export timestamp. | Browser print is not a certified report, and stored modeled/simulated values must remain clearly labeled in any downstream document. |

## Security and Reliability Findings

The following are important limitations rather than unverified accusations. Where a prior independent review suggested a mismatch in OAuth state-cookie attributes, current source inspection shows both creation and clearing use `SameSite=Lax`; that specific mismatch is **not present** in the current code. Structured error handling and public-route restriction now remove the identified local login redirect-loop risk.
...
| S-02 | Resolved locally | Automatic login redirects use typed tRPC `UNAUTHORIZED` handling with a single-flight guard; optional redirect paths are limited to same-origin public routes. | Deployed OAuth callback verification remains an external operator gate. |
...
| S-04 | Partially resolved locally | Worker history is capped, and the reusable MySQL/TiDB pool now bounds both active connections and pending queue work. No measured load benchmark exists. | Add a deterministic high-reading-count regression and measured scaling benchmark before increasing pilot throughput. |
...
| 3 | Explainability Timeline | **Implemented locally.** The Intelligence workspace orders tenant-scoped persisted quality, anomaly, alert, recommendation, and action records, exposing identifiers only when a stored link exists. | Existing IDs, timestamps, and action/recommendation references. | Pure ordering, protected-router, and workspace presentation regressions. |
